...
class ServerMail extends PluginBase implements Listener {
            ...
            case $this->getMessage("commands.names.mail"):
                switch (strtolower(array_shift($args))) {
                    ...
                    case $this->getMessage("commands.names.read"):
                        ...
                        $sender->sendMessage("§b[§eCosmicMail§b]§6 " . sprintf($this->getMessage("messages.count"), count($messages)) . ".");
                    ...
                    case $this->getMessage("commands.names.clear"):
                        ...
                        $sender->sendMessage("§b[§eCosmicMail§b]§6 " . $this->getMessage("messages.cleared"));
                        break;
                    case "send":
                            ...
                            if ($this->checkUser($recipiant)) {
                                if ($this->isMessageSimilar($senderName, $recipiant, $message)) {
                                    ...
                                    if ($msgCount > $msgCountMax) {
                                        $sender->sendMessage("§b[§eCosmicMail§b]§6 " . sprintf($this->getMessage("messages.too_many"), $recipiant) . " (" . ($msgCount - 1) . "/$msgCountMax)");
                                    } else {
                                        ServerMailAPI::addMessage($recipiant, $senderName, $message);
                                        $sender->sendMessage("§b[§eCosmicMail§b]§6 " . $this->getMessage("messages.sent") . " ($msgCount/$msgCountMax)");
                                        $this->sendNotification($recipiant, $senderName);
                                    }
                                }
                            } else {
                                $sender->sendMessage("§b[§eCosmicMail§b]§6 " . sprintf($this->getMessage("messages.no_player"), $recipiant));
                            }
                        } else {
                    ...
                    case $this->getMessage("commands.names.sendall"):
                        if ($sender->hasPermission("tschrock.servermail.command.mail.all")) {
                            ...
                            $sender->sendMessage("§b[§eCosmicMail§b]§6 " . $this->getMessage("messages.sent"));
                            foreach ($this->getServer()->getOnlinePlayers() as $player) {
                                $this->sendNotification($player->getName(), $senderName);
                            }
                        } else {
                            $sender->sendMessage("§b[§eCosmicMail§b]§6" . $this->getMessage("messages.not_allowed"));
                        }
                        break;
                    default:
    ...
    public function sendNotification($player, $sender) {
        if ($this->getConfig()->get(ServerMail::CONFIG_NOTIFY) &&
                ...
                $pPlayer->isOnline()) {
            $pPlayer->sendMessage("§b[§eCosmicMail§b]§6] " . sprintf($this->getMessage("messages.new_message"), $sender));
        }
    }
    ...
    public function onRespawn(PlayerRespawnEvent $event) {
        ...
        $player->sendMessage("§b[§eCosmicMail§b]§6 " . sprintf($this->getMessage("messages.count"), $messagecount) . ".  /"
